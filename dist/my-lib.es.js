...
const buildGrid = () => {
  for (let i = 0; i < rows; i++) {
    ...
    grid == null ? void 0 : grid.appendChild(row);
  }
};
const buildKeyboard = () => {
...
const buildKeyboardRow = (letters, addControls) => {
  let row = document.createElement("div");
  row.className = "flex flex-row justify-center mx-5 my-1";
  if (addControls) {
  ...
  keyboard == null ? void 0 : keyboard.appendChild(row);
};
...
function saveData() {
  let data = JSON.stringify({ secret, historyState: historyWords });
...
let secret = (_a = savedGame == null ? void 0 : savedGame.secret) != null ? _a : WORDS[randomIndex];
let historyWords = (_b = savedGame == null ? void 0 : savedGame.historyState) != null ? _b : [];
let currentAttempt = "";
let successLetters = /* @__PURE__ */ new Set();
let includeLetters = /* @__PURE__ */ new Set();
let keyboardLetters = /* @__PURE__ */ new Map();
const updateGrid = () => {
  let row = grid == null ? void 0 : grid.children[0];
  historyWords.map((attempt) => {
    var _a2;
    drawAttempt(row, attempt, false);
    row = (_a2 = row == null ? void 0 : row.nextElementSibling) != null ? _a2 : void 0;
  });
  if (row) {
    drawAttempt(row, currentAttempt, true);
  }
};
const drawAttempt = (row, attempt, isCurrent) => {
  for (let i = 0; i < columns; i++) {
    let cell = row == null ? void 0 : row.children[i];
    if (cell) {
...
const animatePress = (index2) => {
  let row = grid == null ? void 0 : grid.childNodes[historyWords.length];
  let cell = row == null ? void 0 : row.childNodes[index2];
  cell.setAttribute("data-animation", "pop");
};
const removeAnimatePress = (index2) => {
  let row = grid == null ? void 0 : grid.childNodes[historyWords.length];
  let cell = row == null ? void 0 : row.childNodes[index2];
...
const getCellColor = (attempt, index2) => {
  const correctLetter = secret[index2];
  const attemptLetter = attempt[index2];
